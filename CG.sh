...
echo -e "${purple}============================${rest}"
echo -en "${purple}[Optional] ${green}Enter Your Telegram Channel ID [example: ${yellow}@Ashiweshoonam${green}]: ${rest}"
...
send_to_telegram() {
    local game_name=$1
    local promo_code=$2
    local message="${game_name}: \`${promo_code}\`"
    curl -s -X POST "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage"         -d chat_id="$TELEGRAM_CHANNEL_ID"         -d text="$message"         -d parse_mode="MarkdownV2" > /dev/null 2>&1
}
...
main() {
	...
	while true; do
		for game_choice in {1..9}; do
			...
			if [[ -n "$key" ]]; then
				game_name="${games[$game_choice, name]}"
				message="$game_name : $key"
				echo "$message" | tee -a my_keys.txt
				send_to_telegram "$game_name" "$key"
			else
